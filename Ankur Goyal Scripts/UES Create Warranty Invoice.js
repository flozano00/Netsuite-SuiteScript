...
function beforeSubmitRecord_setWarranty(type){
	/*  On before submit
	 - PURPOSE
	 -
	 FIELDS USED:
	...
	if (type == 'create' || type == 'copy') {
		try {
			...
			var CustCategory = nlapiLookupField('customer', Customer, 'category');

			if (CustCategory == '1' || CustCategory == '22') {
				...
				var DateObj = nlapiStringToDate(InvoiceDate);

				var WarrantyDateObj = nlapiAddDays(DateObj, 30);

				var WarrantyDate = nlapiDateToString(WarrantyDateObj);

				for (var i = 1; i <= LineCount; i++) {
					var TrackWarranty = nlapiGetLineItemValue('item', 'custcol_wrm_reg_hid_trackwarranty', i);
					if (TrackWarranty == 'T') {
						nlapiSelectLineItem('item', i)
						nlapiSetCurrentLineItemValue('item', 'custcol_wrm_reg_warrantyreg', 'T');
						nlapiSetCurrentLineItemValue('item', 'custcol_wrm_reg_hid_registerflag', 'T');
						nlapiSetCurrentLineItemValue('item', 'custcol_wrm_reg_warrantyterms', '4');
						nlapiSetCurrentLineItemValue('item', 'custcol_wrm_reg_hid_warrantyterms', '4');
						nlapiSetCurrentLineItemValue('item', 'custcol_wrm_reg_hid_terms', '4');
						nlapiSetCurrentLineItemValue('item', 'custcol_wrm_reg_warrantyexpire', WarrantyDate);
						nlapiCommitLineItem('item');
					}
				}
			}
